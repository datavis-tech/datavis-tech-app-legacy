...
module.exports = function DocumentRepository(connection) {
    ...
    async function get(id) {
        ...
        return Promise.resolve(serializeDocument(document))
    }

    // TODO test
    async function getReferencesForDocument(id) {
        const document = connection.get(DB_DOCUMENTS_COLLECTION, id)
        const referencesIds = document.data.references.map(r => r.id)
        ...
        return new Promise((resolve, reject) =>
            connection.createFetchQuery(
                ...
                {},
                (err, referenceDocuments) =>
                    err
                        ? reject(err)
                        : resolve(referenceDocuments.map(serializeDocument))
            )
        )
    }
    ...
    function unsubscribe(callback) {
        ...
        if (id && subscribedDocuments[id].callbacks.length === 0) {
            ...
            subscribedDocuments[id].document.removeListener(
                'op',
                sharedbListeners[id]
            )
